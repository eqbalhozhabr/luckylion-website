...
const STRINGS = {
  en: {
    ...
    ui: {
      ...
      hintWhyForcedFog: 'No fire could ever reach it, so it stays lost in the mist — a monster is out there.',
      hintWhyOnlySource: 'That square has to be lit, and only one fire can reach it — so it has to be this one.',
      hintWhyLastStandingFire: (clue) => `A witness said: “${clue}” Every other square nearby is ruled out, so the fire has to be here.`,
      hintWhyLastStandingFog: (clue) => `A witness said: “${clue}” Every other square in reach is already clear, so this one has to hide the monster.`,
      hintWhyFuelSpent: (n) => `All ${n} of your fires are already spoken for elsewhere, so this square cannot have one.`,
      hintWhyFuelForced: 'Exactly as many open squares are left as fires you still have — every one of them has to be lit.',
      hintWhyMonsterCountFog: (clue) => `A witness said: “${clue}” That count only works if this square hides a monster too.`,
      hintWhyMonsterCountClear: (clue) => `A witness said: “${clue}” That count is already met, so every other square must be clear.`,
      hintWhyScorchFire: 'The scorch mark on that rock needs one more fire touching it, and only this square can still give it that.',
      hintWhyScorchNoFire: 'The scorch mark on that rock already has all the fire it can take — no more here.',
      hintWhyDisjoint: 'A few safe squares nearby cannot all share one fire, and that already accounts for the rest of your fires — none are left for here.',
      hintWhyForward: 'Try the opposite here and the testimony falls apart — so this is the only way it holds together.',
      hintWrongFireCited: (num, clue) => `That fire is wrong — testimony #${num} said: “${clue}” This square should stay safe.`,
      hintWrongFireBecause: (reason) => `That fire is wrong. ${reason}`,
      hintWrongFire: 'That fire does not match the survivors’ testimony.',
  ...
  fa: {
    ...
    ui: {
      ...
      hintWhyForcedFog: 'هیچ آتشی هرگز به آن‌جا نمی‌رسد، پس در مه می‌ماند — هیولایی همان‌جاست.',
      hintWhyOnlySource: 'آن خانه باید روشن شود، و فقط یک آتش می‌تواند به آن برسد — پس دقیقاً همان‌جاست.',
      hintWhyLastStandingFire: (clue) => `شاهدی گفت: «${clue}» بقیه‌ی خانه‌های اطراف رد شده‌اند، پس آتش باید همین‌جا باشد.`,
      hintWhyLastStandingFog: (clue) => `شاهدی گفت: «${clue}» بقیه‌ی خانه‌های آن محدوده از قبل امن شده‌اند، پس هیولا باید همین‌جا باشد.`,
      hintWhyFuelSpent: (n) => `هر ${n} آتشت از قبل جای دیگری مصرف شده، پس این خانه دیگر نمی‌تواند آتش داشته باشد.`,
      hintWhyFuelForced: 'دقیقاً به‌اندازه‌ی آتش‌های باقی‌مانده‌ات خانه‌ی خالی باقی مانده — همه‌شان باید روشن شوند.',
      hintWhyMonsterCountFog: (clue) => `شاهدی گفت: «${clue}» این شمارش فقط وقتی درست درمی‌آید که اینجا هم هیولا باشد.`,
      hintWhyMonsterCountClear: (clue) => `شاهدی گفت: «${clue}» این شمارش از قبل کامل شده، پس بقیه‌ی خانه‌ها باید امن باشند.`,
      hintWhyScorchFire: 'نشان سوختگی روی آن سنگ یک آتش دیگر لازم دارد، و فقط همین خانه می‌تواند آن را بدهد.',
      hintWhyScorchNoFire: 'نشان سوختگی روی آن سنگ از قبل کامل شده — آتش دیگری اینجا لازم نیست.',
      hintWhyDisjoint: 'چند خانه‌ی امن نزدیک هم را هیچ آتش مشترکی نمی‌تواند پوشش دهد، و همین حالا بقیه‌ی آتش‌هایت را مصرف کرده — اینجا دیگر جا نیست.',
      hintWhyForward: 'اگر اینجا برعکس بود، با گزارش‌ها جور درنمی‌آمد — پس همین درست است.',
      hintWrongFireCited: (num, clue) => `این آتش اشتباه است — شاهد شماره‌ی ${num} گفت: «${clue}» این خانه باید امن بماند.`,
      hintWrongFireBecause: (reason) => `این آتش اشتباه است. ${reason}`,
      hintWrongFire: 'این آتش با گزارش‌های بازمانده‌ها جور درنمی‌آید.',
  ...
  ar: {
    ...
    ui: {
      ...
      hintWhyForcedFog: 'لا نار يمكن أن تصل إلى هناك أبداً، فيبقى ذلك المربع ضائعاً في الضباب — يوجد وحش هناك.',
      hintWhyOnlySource: 'ذلك المربع يجب أن يُضاء، ونار واحدة فقط يمكنها الوصول إليه — فلا بد أنها هذه.',
      hintWhyLastStandingFire: (clue) => `قال أحد الناجين: «${clue}» كل المربعات الأخرى القريبة استُبعدت، فلا بد أن تكون النار هنا.`,
      hintWhyLastStandingFog: (clue) => `قال أحد الناجين: «${clue}» كل المربعات الأخرى في المدى أصبحت آمنة، فلا بد أن الوحش هنا.`,
      hintWhyFuelSpent: (n) => `كل النيران الـ${n} التي تملكها استُخدمت في مكان آخر، فهذا المربع لا يمكن أن يحمل ناراً.`,
      hintWhyFuelForced: 'بقي بالضبط عدد المربعات المفتوحة بقدر عدد النيران المتبقية لديك — كلها يجب أن تُضاء.',
      hintWhyMonsterCountFog: (clue) => `قال أحد الناجين: «${clue}» هذا العدد لا يصح إلا إذا كان هنا وحش أيضاً.`,
      hintWhyMonsterCountClear: (clue) => `قال أحد الناجين: «${clue}» هذا العدد اكتمل بالفعل، فبقية المربعات يجب أن تكون آمنة.`,
      hintWhyScorchFire: 'علامة الاحتراق على تلك الصخرة تحتاج ناراً واحدة إضافية، ولا يمكن أن تأتي إلا من هذا المربع.',
      hintWhyScorchNoFire: 'علامة الاحتراق على تلك الصخرة اكتملت بالفعل — لا حاجة لنار إضافية هنا.',
      hintWhyDisjoint: 'عدة مربعات آمنة قريبة لا يمكن أن تغطيها نار واحدة مشتركة، وهذا يستهلك بالفعل بقية نيرانك — لا مكان لها هنا.',
      hintWhyForward: 'لو كان العكس صحيحاً هنا، لتناقضت الشهادات — فهذا هو الوضع الوحيد الممكن.',
      hintWrongFireCited: (num, clue) => `هذه النار خاطئة — الشهادة رقم ${num} قالت: «${clue}» يجب أن يبقى هذا المربع آمناً.`,
      hintWrongFireBecause: (reason) => `هذه النار خاطئة. ${reason}`,
      hintWrongFire: 'هذه النار لا تتفق مع شهادات الناجين.',
  ...
  zh: {
    ...
    ui: {
      ...
      hintWhyForcedFog: '没有任何篝火能照到这里，所以它会一直笼罩在迷雾中——那里有怪物。',
      hintWhyOnlySource: '那个格子必须被点亮，而只有一处篝火能照到它——所以就是这里。',
      hintWhyLastStandingFire: (clue) => `有幸存者说：「${clue}」附近其他格子都已排除，所以篝火必须在这里。`,
      hintWhyLastStandingFog: (clue) => `有幸存者说：「${clue}」范围内其他格子都已安全，所以怪物必须在这里。`,
      hintWhyFuelSpent: (n) => `你的${n}处篝火都已经用在别处了，这个格子不可能再有篝火。`,
      hintWhyFuelForced: '剩下的空格数量正好等于你剩余的篝火数——它们必须全部点亮。',
      hintWhyMonsterCountFog: (clue) => `有幸存者说：「${clue}」只有这里也藏着怪物，这个数字才对得上。`,
      hintWhyMonsterCountClear: (clue) => `有幸存者说：「${clue}」这个数字已经凑齐了，其余格子必须是安全的。`,
      hintWhyScorchFire: '那块岩石上的焦痕还需要一处篝火，而只有这个格子能满足它。',
      hintWhyScorchNoFire: '那块岩石上的焦痕已经满了——这里不能再有篝火。',
      hintWhyDisjoint: '附近几个安全格子无法被同一处篝火同时照到，这已经用光了你剩下的篝火——这里分不到了。',
      hintWhyForward: '如果这里反过来，就会和证言相矛盾——所以只能是这样。',
      hintWrongFireCited: (num, clue) => `这处篝火错了——第${num}条证言说：「${clue}」这个格子应该保持安全。`,
      hintWrongFireBecause: (reason) => `这处篝火错了。${reason}`,
      hintWrongFire: '这处篝火和幸存者的证言对不上。',
  ...
  de: {
    ...
    ui: {
      ...
      hintWhyForcedFog: 'Kein Feuer kann dieses Feld je erreichen, also bleibt es im Nebel — dort lauert ein Monster.',
      hintWhyOnlySource: 'Dieses Feld muss beleuchtet werden, und nur ein Feuer kann es erreichen — also muss es dieses sein.',
      hintWhyLastStandingFire: (clue) => `Ein Überlebender sagte: „${clue}“ Alle anderen Felder in der Nähe sind ausgeschlossen, also muss das Feuer hier stehen.`,
      hintWhyLastStandingFog: (clue) => `Ein Überlebender sagte: „${clue}“ Alle anderen Felder in Reichweite sind schon sicher, also muss hier das Monster sein.`,
      hintWhyFuelSpent: (n) => `Alle ${n} deiner Feuer sind schon anderswo vergeben, dieses Feld kann also keins haben.`,
      hintWhyFuelForced: 'Es sind genau so viele freie Felder übrig wie Feuer, die dir noch bleiben — alle müssen brennen.',
      hintWhyMonsterCountFog: (clue) => `Ein Überlebender sagte: „${clue}“ Diese Zahl stimmt nur, wenn hier auch ein Monster steckt.`,
      hintWhyMonsterCountClear: (clue) => `Ein Überlebender sagte: „${clue}“ Diese Zahl ist schon erreicht, also müssen alle anderen Felder sicher sein.`,
      hintWhyScorchFire: 'Die Brandspur an diesem Felsen braucht noch ein Feuer, und nur dieses Feld kann es liefern.',
      hintWhyScorchNoFire: 'Die Brandspur an diesem Felsen hat schon alles Feuer, das sie verträgt — hier keins mehr.',
      hintWhyDisjoint: 'Mehrere sichere Felder in der Nähe können sich kein gemeinsames Feuer teilen, das bindet schon den Rest deiner Feuer — hier bleibt keins übrig.',
      hintWhyForward: 'Wäre es hier umgekehrt, würden die Aussagen nicht mehr zusammenpassen — also muss es so sein.',
      hintWrongFireCited: (num, clue) => `Dieses Feuer ist falsch — Aussage Nr. ${num} sagte: „${clue}“ Dieses Feld sollte sicher bleiben.`,
      hintWrongFireBecause: (reason) => `Dieses Feuer ist falsch. ${reason}`,
      hintWrongFire: 'Dieses Feuer passt nicht zu den Aussagen der Überlebenden.',
  ...
  es: {
    ...
    ui: {
      ...
      hintWhyForcedFog: 'Ningún fuego puede alcanzar jamás esa casilla, así que se queda en la niebla — hay un monstruo ahí.',
      hintWhyOnlySource: 'Esa casilla tiene que iluminarse, y solo un fuego puede alcanzarla — así que tiene que ser este.',
      hintWhyLastStandingFire: (clue) => `Un superviviente dijo: «${clue}» Las demás casillas cercanas ya están descartadas, así que el fuego tiene que ir aquí.`,
      hintWhyLastStandingFog: (clue) => `Un superviviente dijo: «${clue}» Las demás casillas a su alcance ya están despejadas, así que el monstruo tiene que estar aquí.`,
      hintWhyFuelSpent: (n) => `Tus ${n} fuegos ya están colocados en otro sitio, así que esta casilla no puede tener ninguno.`,
      hintWhyFuelForced: 'Quedan exactamente tantas casillas abiertas como fuegos te faltan por colocar — todas tienen que encenderse.',
      hintWhyMonsterCountFog: (clue) => `Un superviviente dijo: «${clue}» Esa cuenta solo cuadra si aquí también hay un monstruo.`,
      hintWhyMonsterCountClear: (clue) => `Un superviviente dijo: «${clue}» Esa cuenta ya se cumplió, así que las demás casillas deben estar despejadas.`,
      hintWhyScorchFire: 'La marca de quemadura en esa roca necesita un fuego más, y solo esta casilla puede dárselo.',
      hintWhyScorchNoFire: 'La marca de quemadura en esa roca ya tiene todo el fuego que admite — ninguno más aquí.',
      hintWhyDisjoint: 'Varias casillas seguras cercanas no pueden compartir un mismo fuego, y eso ya ocupa el resto de tus fuegos — aquí no queda ninguno.',
      hintWhyForward: 'Si aquí fuera al revés, los testimonios dejarían de encajar — así que solo puede ser así.',
      hintWrongFireCited: (num, clue) => `Ese fuego está mal — el testimonio n.º ${num} dijo: «${clue}» Esta casilla debería quedar a salvo.`,
      hintWrongFireBecause: (reason) => `Ese fuego está mal. ${reason}`,
      hintWrongFire: 'Ese fuego no coincide con el testimonio de los supervivientes.',
  ...
  fr: {
    ...
    ui: {
      ...
      hintWhyForcedFog: 'Aucun feu ne peut jamais atteindre cette case, elle reste donc dans la brume — un monstre s\'y trouve.',
      hintWhyOnlySource: 'Cette case doit être éclairée, et un seul feu peut l\'atteindre — ce doit donc être celui-là.',
      hintWhyLastStandingFire: (clue) => `Un survivant a dit : « ${clue} » Toutes les autres cases voisines sont écartées, le feu doit donc être ici.`,
      hintWhyLastStandingFog: (clue) => `Un survivant a dit : « ${clue} » Toutes les autres cases à portée sont déjà dégagées, le monstre doit donc être ici.`,
      hintWhyFuelSpent: (n) => `Vos ${n} feux sont déjà placés ailleurs, cette case ne peut donc pas en avoir un.`,
      hintWhyFuelForced: 'Il reste exactement autant de cases libres que de feux à placer — elles doivent toutes être allumées.',
      hintWhyMonsterCountFog: (clue) => `Un survivant a dit : « ${clue} » Ce compte ne fonctionne que si cette case cache aussi un monstre.`,
      hintWhyMonsterCountClear: (clue) => `Un survivant a dit : « ${clue} » Ce compte est déjà atteint, toutes les autres cases doivent donc être sûres.`,
      hintWhyScorchFire: 'La trace de brûlure sur ce rocher a besoin d\'un feu de plus, et seule cette case peut le lui donner.',
      hintWhyScorchNoFire: 'La trace de brûlure sur ce rocher a déjà tout le feu qu\'elle peut recevoir — plus aucun ici.',
      hintWhyDisjoint: 'Plusieurs cases sûres voisines ne peuvent pas partager un même feu, ce qui occupe déjà le reste de vos feux — il n\'en reste aucun pour ici.',
      hintWhyForward: 'Si c\'était l\'inverse ici, les témoignages ne concorderaient plus — c\'est donc la seule possibilité.',
      hintWrongFireCited: (num, clue) => `Ce feu est faux — le témoignage n° ${num} disait : « ${clue} » Cette case devrait rester sûre.`,
      hintWrongFireBecause: (reason) => `Ce feu est faux. ${reason}`,
      hintWrongFire: 'Ce feu ne correspond pas aux témoignages des survivants.',
  ...
  it: {
    ...
    ui: {
      ...
      hintWhyForcedFog: 'Nessun fuoco può mai raggiungere questa casella, quindi resta nella nebbia — c\'è un mostro lì.',
      hintWhyOnlySource: 'Quella casella deve essere illuminata, e solo un fuoco può raggiungerla — quindi deve essere questo.',
      hintWhyLastStandingFire: (clue) => `Un sopravvissuto ha detto: «${clue}» Tutte le altre caselle vicine sono escluse, quindi il fuoco deve essere qui.`,
      hintWhyLastStandingFog: (clue) => `Un sopravvissuto ha detto: «${clue}» Tutte le altre caselle a portata sono già sicure, quindi il mostro deve essere qui.`,
      hintWhyFuelSpent: (n) => `Tutti i tuoi ${n} fuochi sono già piazzati altrove, quindi questa casella non può averne uno.`,
      hintWhyFuelForced: 'Restano esattamente tante caselle libere quanti fuochi ti restano — devono accendersi tutte.',
      hintWhyMonsterCountFog: (clue) => `Un sopravvissuto ha detto: «${clue}» Quel conto torna solo se anche qui c'è un mostro.`,
      hintWhyMonsterCountClear: (clue) => `Un sopravvissuto ha detto: «${clue}» Quel conto è già raggiunto, quindi tutte le altre caselle devono essere sicure.`,
      hintWhyScorchFire: 'Il segno di bruciatura su quella roccia ha bisogno di un altro fuoco, e solo questa casella può darglielo.',
      hintWhyScorchNoFire: 'Il segno di bruciatura su quella roccia ha già tutto il fuoco che può ricevere — nessun altro qui.',
      hintWhyDisjoint: 'Diverse caselle sicure vicine non possono condividere un solo fuoco, il che occupa già il resto dei tuoi fuochi — qui non ne resta nessuno.',
      hintWhyForward: 'Se qui fosse il contrario, le testimonianze non tornerebbero più — quindi può essere solo così.',
      hintWrongFireCited: (num, clue) => `Quel fuoco è sbagliato — la testimonianza n. ${num} diceva: «${clue}» Questa casella dovrebbe restare sicura.`,
      hintWrongFireBecause: (reason) => `Quel fuoco è sbagliato. ${reason}`,
      hintWrongFire: 'Quel fuoco non corrisponde alle testimonianze dei sopravvissuti.',
  ...
  pl: {
    ...
    ui: {
      ...
      hintWhyForcedFog: 'Żaden ogień nigdy tam nie dotrze, więc to pole zostaje we mgle — jest tam potwór.',
      hintWhyOnlySource: 'To pole musi zostać oświetlone, a tylko jeden ogień może je dosięgnąć — więc musi być właśnie tutaj.',
      hintWhyLastStandingFire: (clue) => `Ocalały powiedział: „${clue}” Wszystkie inne pobliskie pola są już wykluczone, więc ogień musi być tutaj.`,
      hintWhyLastStandingFog: (clue) => `Ocalały powiedział: „${clue}” Wszystkie inne pola w zasięgu są już bezpieczne, więc potwór musi być tutaj.`,
      hintWhyFuelSpent: (n) => `Wszystkie twoje ${n} ognie są już rozstawione gdzie indziej, więc to pole nie może mieć ognia.`,
      hintWhyFuelForced: 'Zostało dokładnie tyle wolnych pól, ile masz jeszcze ogni — wszystkie muszą zostać rozpalone.',
      hintWhyMonsterCountFog: (clue) => `Ocalały powiedział: „${clue}” Ta liczba zgadza się tylko wtedy, gdy tutaj też jest potwór.`,
      hintWhyMonsterCountClear: (clue) => `Ocalały powiedział: „${clue}” Ta liczba już się zgadza, więc wszystkie inne pola muszą być bezpieczne.`,
      hintWhyScorchFire: 'Ślad przypalenia na tej skale wymaga jeszcze jednego ognia, a tylko to pole może go dać.',
      hintWhyScorchNoFire: 'Ślad przypalenia na tej skale ma już tyle ognia, ile może przyjąć — tutaj już żadnego więcej.',
      hintWhyDisjoint: 'Kilka pobliskich bezpiecznych pól nie może dzielić jednego ognia, co już pochłania resztę twoich ogni — tu nic nie zostało.',
      hintWhyForward: 'Gdyby tu było odwrotnie, zeznania przestałyby się zgadzać — więc może być tylko tak.',
      hintWrongFireCited: (num, clue) => `Ten ogień jest błędny — zeznanie nr ${num} mówiło: „${clue}” To pole powinno pozostać bezpieczne.`,
      hintWrongFireBecause: (reason) => `Ten ogień jest błędny. ${reason}`,
      hintWrongFire: 'Ten ogień nie zgadza się z zeznaniami ocalałych.',
